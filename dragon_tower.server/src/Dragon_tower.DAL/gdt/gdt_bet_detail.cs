...
namespace Dragon_tower.DAL
{
	...
	/// <summary>
	/// 下注表
	/// 【表 gdt_bet_detail 的实体类】
	/// </summary>
	[DataContract]
	[Obsolete]
	public class Gdt_bet_detailEO : IRowMapper<Gdt_bet_detailEO>
	{
		/// <summary>
		/// 构造函数 
		/// </summary>
		public Gdt_bet_detailEO()
		{
			this.BetAmount = 0;
			this.WinAmount = 0;
			this.Multip = 0d;
		...
		/// <return>实体对象</return>
		public Gdt_bet_detailEO MapRow(IDataReader reader)
		{
		...
		/// <param name = "reader">只进结果集流</param>
		/// <return>实体对象</return>
		public static Gdt_bet_detailEO MapDataReader(IDataReader reader)
		{
		    Gdt_bet_detailEO ret = new Gdt_bet_detailEO();
			...
			ret.CellOpenId = reader.ToInt32("CellOpenId");
	...
	/// <summary>
	/// 下注表
	/// 【表 gdt_bet_detail 的操作类】
	/// </summary>
	[Obsolete]
	public class Gdt_bet_detailMO : MySqlTableMO<Gdt_bet_detailEO>
	{
		/// <summary>
		/// 表名
		/// </summary>
	    public override string TableName { get; set; } = "`gdt_bet_detail`";
		...
		public Gdt_bet_detailMO(MySqlDatabase database) : base(database) { }
		/// <summary>
		/// 构造函数
		/// </summary>
		/// <param name = "connectionStringName">配置文件.config中定义的连接字符串名称</param>
		public Gdt_bet_detailMO(string connectionStringName = null) : base(connectionStringName) { }
	    /// <summary>
	    /// 构造函数
	    /// </summary>
	    /// <param name="connectionString">数据库连接字符串，如server=192.168.1.1;database=testdb;uid=root;pwd=root</param>
	    /// <param name="commandTimeout">CommandTimeout时间</param>
	    public Gdt_bet_detailMO(string connectionString, int commandTimeout) : base(connectionString, commandTimeout) { }
		...
		public override int Add(Gdt_bet_detailEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_); 
		}
		public override async Task<int> AddAsync(Gdt_bet_detailEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_); 
		}
	    private void RepairAddData(Gdt_bet_detailEO item, bool useIgnore_, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = useIgnore_ ? "INSERT IGNORE" : "INSERT";
			sql_ += $" INTO {TableName} (`BetID`, `UserID`, `BetAmount`, `WinAmount`, `FloorNum`, `IsWin`, `Multip`, `CellOpenId`, `CellType`, `OrderStatus`, `RecDate`) VALUE (@BetID, @UserID, @BetAmount, @WinAmount, @FloorNum, @IsWin, @Multip, @CellOpenId, @CellType, @OrderStatus, @RecDate);";
			paras_ = new List<MySqlParameter>() 
			{
				...
				Database.CreateInParameter("@CellOpenId", item.CellOpenId, MySqlDbType.Int32),
		...
		public int AddByBatch(IEnumerable<Gdt_bet_detailEO> items, int batchCount, TransactionManager tm_ = null)
		{
			var ret = 0;
	    ...
	    public async Task<int> AddByBatchAsync(IEnumerable<Gdt_bet_detailEO> items, int batchCount, TransactionManager tm_ = null)
	    {
	        var ret = 0;
	        foreach (var sql in BuildAddBatchSql(items, batchCount))
	        {
	    ...
	    private IEnumerable<string> BuildAddBatchSql(IEnumerable<Gdt_bet_detailEO> items, int batchCount)
		{
			var count = 0;
	        var insertSql = $"INSERT INTO {TableName} (`BetID`, `UserID`, `BetAmount`, `WinAmount`, `FloorNum`, `IsWin`, `Multip`, `CellOpenId`, `CellType`, `OrderStatus`, `RecDate`) VALUES ";
			var sql = new StringBuilder();
	        foreach (var item in items)
			{
				count++;
				sql.Append($"({item.BetID},'{item.UserID}',{item.BetAmount},{item.WinAmount},{item.FloorNum},{item.IsWin},{item.Multip},{item.CellOpenId},{item.CellType},{item.OrderStatus},'{item.RecDate?.ToString("yyyy-MM-dd HH:mm:ss")}'),");
				if (count == batchCount)
				{
					count = 0;
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Remove(Gdt_bet_detailEO item, TransactionManager tm_ = null)
		{
			return RemoveByPK(item.BetID, tm_);
		}
		public async Task<int> RemoveAsync(Gdt_bet_detailEO item, TransactionManager tm_ = null)
		{
		...
		/// 按字段删除
		/// </summary>
		/// /// <param name = "winAmount">赢钱金额</param>
		/// <param name="tm_">事务管理对象</param>
		...
		#region RemoveByCellOpenId
		/// <summary>
		/// 按字段删除
		/// </summary>
		/// /// <param name = "cellOpenId">开启的格子Id</param>
		/// <param name="tm_">事务管理对象</param>
		public int RemoveByCellOpenId(int cellOpenId, TransactionManager tm_ = null)
		{
			RepairRemoveByCellOpenIdData(cellOpenId, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> RemoveByCellOpenIdAsync(int cellOpenId, TransactionManager tm_ = null)
		{
			RepairRemoveByCellOpenIdData(cellOpenId, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairRemoveByCellOpenIdData(int cellOpenId, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"DELETE FROM {TableName} WHERE `CellOpenId` = @CellOpenId";
			paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@CellOpenId", cellOpenId, MySqlDbType.Int32));
		}
		#endregion // RemoveByCellOpenId
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Put(Gdt_bet_detailEO item, TransactionManager tm_ = null)
		{
			RepairPutData(item, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutAsync(Gdt_bet_detailEO item, TransactionManager tm_ = null)
		{
			RepairPutData(item, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutData(Gdt_bet_detailEO item, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `BetID` = @BetID, `UserID` = @UserID, `BetAmount` = @BetAmount, `WinAmount` = @WinAmount, `FloorNum` = @FloorNum, `IsWin` = @IsWin, `Multip` = @Multip, `CellOpenId` = @CellOpenId, `CellType` = @CellType, `OrderStatus` = @OrderStatus WHERE `BetID` = @BetID_Original";
			paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@BetID", item.BetID, MySqlDbType.Int64),
				Database.CreateInParameter("@UserID", item.UserID, MySqlDbType.VarChar),
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Put(IEnumerable<Gdt_bet_detailEO> items, TransactionManager tm_ = null)
		{
			int ret = 0;
			foreach (var item in items)
			{
		...
		public async Task<int> PutAsync(IEnumerable<Gdt_bet_detailEO> items, TransactionManager tm_ = null)
		{
			int ret = 0;
			foreach (var item in items)
			{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutFloorNumByPK(long betID, int floorNum, TransactionManager tm_ = null)
		{
			RepairPutFloorNumByPKData(betID, floorNum, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutFloorNumByPKAsync(long betID, int floorNum, TransactionManager tm_ = null)
		{
			RepairPutFloorNumByPKData(betID, floorNum, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutFloorNumByPKData(long betID, int floorNum, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `FloorNum` = @FloorNum  WHERE `BetID` = @BetID";
			paras_ = new List<MySqlParameter>() 
			{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutCellOpenIdByPK(long betID, int cellOpenId, TransactionManager tm_ = null)
		{
			RepairPutCellOpenIdByPKData(betID, cellOpenId, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutCellOpenIdByPKAsync(long betID, int cellOpenId, TransactionManager tm_ = null)
		{
			RepairPutCellOpenIdByPKData(betID, cellOpenId, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutCellOpenIdByPKData(long betID, int cellOpenId, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `CellOpenId` = @CellOpenId  WHERE `BetID` = @BetID";
			paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@CellOpenId", cellOpenId, MySqlDbType.Int32),
				Database.CreateInParameter("@BetID", betID, MySqlDbType.Int64),
			};
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutCellOpenId(int cellOpenId, TransactionManager tm_ = null)
		{
			string sql_ = $"UPDATE {TableName} SET `CellOpenId` = @CellOpenId";
			var parameter_ = Database.CreateInParameter("@CellOpenId", cellOpenId, MySqlDbType.Int32);
			return Database.ExecSqlNonQuery(sql_, new MySqlParameter[] { parameter_ }, tm_);
		}
		public async Task<int> PutCellOpenIdAsync(int cellOpenId, TransactionManager tm_ = null)
		{
			string sql_ = $"UPDATE {TableName} SET `CellOpenId` = @CellOpenId";
			var parameter_ = Database.CreateInParameter("@CellOpenId", cellOpenId, MySqlDbType.Int32);
			return await Database.ExecSqlNonQueryAsync(sql_, new MySqlParameter[] { parameter_ }, tm_);
		}
		#endregion // PutCellOpenId
		#region PutCellType
		/// <summary>
		/// 按主键更新列数据
		/// </summary>
		...
		public bool Set(Gdt_bet_detailEO item, TransactionManager tm = null)
		{
			bool ret = true;
		...
		public async Task<bool> SetAsync(Gdt_bet_detailEO item, TransactionManager tm = null)
		{
			bool ret = true;
			if(GetByPK(item.BetID) == null)
			{
		...
		/// <return></return>
		public Gdt_bet_detailEO GetByPK(long betID, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(betID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return Database.ExecSqlSingle(sql_, paras_, tm_, Gdt_bet_detailEO.MapDataReader);
		}
		public async Task<Gdt_bet_detailEO> GetByPKAsync(long betID, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(betID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return await Database.ExecSqlSingleAsync(sql_, paras_, tm_, Gdt_bet_detailEO.MapDataReader);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return></return>
		public int GetFloorNumByPK(long betID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@BetID", betID, MySqlDbType.Int64),
			};
			return (int)GetScalar("`FloorNum`", "`BetID` = @BetID", paras_, tm_);
		}
		public async Task<int> GetFloorNumByPKAsync(long betID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@BetID", betID, MySqlDbType.Int64),
			};
			return (int)await GetScalarAsync("`FloorNum`", "`BetID` = @BetID", paras_, tm_);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return></return>
		public int GetCellOpenIdByPK(long betID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@BetID", betID, MySqlDbType.Int64),
			};
			return (int)GetScalar("`CellOpenId`", "`BetID` = @BetID", paras_, tm_);
		}
		public async Task<int> GetCellOpenIdByPKAsync(long betID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@BetID", betID, MySqlDbType.Int64),
			};
			return (int)await GetScalarAsync("`CellOpenId`", "`BetID` = @BetID", paras_, tm_);
		}
		...
		public List<Gdt_bet_detailEO> GetByUserID(string userID)
		{
			return GetByUserID(userID, 0, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByUserIDAsync(string userID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByUserID(string userID, TransactionManager tm_)
		{
			return GetByUserID(userID, 0, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByUserIDAsync(string userID, TransactionManager tm_)
		{
		...
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByUserID(string userID, int top_)
		{
			return GetByUserID(userID, top_, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByUserIDAsync(string userID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByUserID(string userID, int top_, TransactionManager tm_)
		{
			return GetByUserID(userID, top_, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByUserIDAsync(string userID, int top_, TransactionManager tm_)
		{
		...
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByUserID(string userID, string sort_)
		{
			return GetByUserID(userID, 0, sort_, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByUserIDAsync(string userID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByUserID(string userID, string sort_, TransactionManager tm_)
		{
			return GetByUserID(userID, 0, sort_, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByUserIDAsync(string userID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByUserID(string userID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`UserID` = @UserID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@UserID", userID, MySqlDbType.VarChar));
			return Database.ExecSqlList(sql_, paras_, tm_, Gdt_bet_detailEO.MapDataReader);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByUserIDAsync(string userID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`UserID` = @UserID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@UserID", userID, MySqlDbType.VarChar));
			return await Database.ExecSqlListAsync(sql_, paras_, tm_, Gdt_bet_detailEO.MapDataReader);
		}
		...
		/// /// <param name = "betAmount">实际下注金额</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByBetAmount(long betAmount)
		{
			return GetByBetAmount(betAmount, 0, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByBetAmountAsync(long betAmount)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByBetAmount(long betAmount, TransactionManager tm_)
		{
			return GetByBetAmount(betAmount, 0, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByBetAmountAsync(long betAmount, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByBetAmount(long betAmount, int top_)
		{
			return GetByBetAmount(betAmount, top_, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByBetAmountAsync(long betAmount, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByBetAmount(long betAmount, int top_, TransactionManager tm_)
		{
			return GetByBetAmount(betAmount, top_, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByBetAmountAsync(long betAmount, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByBetAmount(long betAmount, string sort_)
		{
			return GetByBetAmount(betAmount, 0, sort_, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByBetAmountAsync(long betAmount, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByBetAmount(long betAmount, string sort_, TransactionManager tm_)
		{
			return GetByBetAmount(betAmount, 0, sort_, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByBetAmountAsync(long betAmount, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByBetAmount(long betAmount, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Gdt_bet_detailEO>> GetByBetAmountAsync(long betAmount, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`BetAmount` = @BetAmount", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@BetAmount", betAmount, MySqlDbType.Int64));
		...
		/// /// <param name = "winAmount">赢钱金额</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByWinAmount(long winAmount)
		{
			return GetByWinAmount(winAmount, 0, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByWinAmountAsync(long winAmount)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByWinAmount(long winAmount, TransactionManager tm_)
		{
			return GetByWinAmount(winAmount, 0, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByWinAmountAsync(long winAmount, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByWinAmount(long winAmount, int top_)
		{
			return GetByWinAmount(winAmount, top_, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByWinAmountAsync(long winAmount, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByWinAmount(long winAmount, int top_, TransactionManager tm_)
		{
			return GetByWinAmount(winAmount, top_, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByWinAmountAsync(long winAmount, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByWinAmount(long winAmount, string sort_)
		{
			return GetByWinAmount(winAmount, 0, sort_, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByWinAmountAsync(long winAmount, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByWinAmount(long winAmount, string sort_, TransactionManager tm_)
		{
			return GetByWinAmount(winAmount, 0, sort_, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByWinAmountAsync(long winAmount, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByWinAmount(long winAmount, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Gdt_bet_detailEO>> GetByWinAmountAsync(long winAmount, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`WinAmount` = @WinAmount", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@WinAmount", winAmount, MySqlDbType.Int64));
		...
		/// /// <param name = "floorNum">第几层</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByFloorNum(int floorNum)
		{
			return GetByFloorNum(floorNum, 0, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByFloorNumAsync(int floorNum)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByFloorNum(int floorNum, TransactionManager tm_)
		{
			return GetByFloorNum(floorNum, 0, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByFloorNumAsync(int floorNum, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByFloorNum(int floorNum, int top_)
		{
			return GetByFloorNum(floorNum, top_, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByFloorNumAsync(int floorNum, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByFloorNum(int floorNum, int top_, TransactionManager tm_)
		{
			return GetByFloorNum(floorNum, top_, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByFloorNumAsync(int floorNum, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByFloorNum(int floorNum, string sort_)
		{
			return GetByFloorNum(floorNum, 0, sort_, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByFloorNumAsync(int floorNum, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByFloorNum(int floorNum, string sort_, TransactionManager tm_)
		{
			return GetByFloorNum(floorNum, 0, sort_, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByFloorNumAsync(int floorNum, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByFloorNum(int floorNum, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Gdt_bet_detailEO>> GetByFloorNumAsync(int floorNum, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`FloorNum` = @FloorNum", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@FloorNum", floorNum, MySqlDbType.Int32));
		...
		/// /// <param name = "isWin">是否中奖</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByIsWin(int isWin)
		{
			return GetByIsWin(isWin, 0, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByIsWinAsync(int isWin)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByIsWin(int isWin, TransactionManager tm_)
		{
			return GetByIsWin(isWin, 0, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByIsWinAsync(int isWin, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByIsWin(int isWin, int top_)
		{
			return GetByIsWin(isWin, top_, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByIsWinAsync(int isWin, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByIsWin(int isWin, int top_, TransactionManager tm_)
		{
			return GetByIsWin(isWin, top_, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByIsWinAsync(int isWin, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByIsWin(int isWin, string sort_)
		{
			return GetByIsWin(isWin, 0, sort_, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByIsWinAsync(int isWin, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByIsWin(int isWin, string sort_, TransactionManager tm_)
		{
			return GetByIsWin(isWin, 0, sort_, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByIsWinAsync(int isWin, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByIsWin(int isWin, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Gdt_bet_detailEO>> GetByIsWinAsync(int isWin, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`IsWin` = @IsWin", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@IsWin", isWin, MySqlDbType.Int32));
		...
		/// /// <param name = "multip">倍数</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByMultip(double multip)
		{
			return GetByMultip(multip, 0, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByMultipAsync(double multip)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByMultip(double multip, TransactionManager tm_)
		{
			return GetByMultip(multip, 0, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByMultipAsync(double multip, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByMultip(double multip, int top_)
		{
			return GetByMultip(multip, top_, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByMultipAsync(double multip, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByMultip(double multip, int top_, TransactionManager tm_)
		{
			return GetByMultip(multip, top_, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByMultipAsync(double multip, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByMultip(double multip, string sort_)
		{
			return GetByMultip(multip, 0, sort_, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByMultipAsync(double multip, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByMultip(double multip, string sort_, TransactionManager tm_)
		{
			return GetByMultip(multip, 0, sort_, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByMultipAsync(double multip, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByMultip(double multip, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Gdt_bet_detailEO>> GetByMultipAsync(double multip, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`Multip` = @Multip", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@Multip", multip, MySqlDbType.Double));
		...
		/// /// <param name = "cellOpenId">开启的格子Id</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByCellOpenId(int cellOpenId)
		{
			return GetByCellOpenId(cellOpenId, 0, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByCellOpenIdAsync(int cellOpenId)
		{
			return await GetByCellOpenIdAsync(cellOpenId, 0, string.Empty, null);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByCellOpenId(int cellOpenId, TransactionManager tm_)
		{
			return GetByCellOpenId(cellOpenId, 0, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByCellOpenIdAsync(int cellOpenId, TransactionManager tm_)
		{
			return await GetByCellOpenIdAsync(cellOpenId, 0, string.Empty, tm_);
		}
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByCellOpenId(int cellOpenId, int top_)
		{
			return GetByCellOpenId(cellOpenId, top_, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByCellOpenIdAsync(int cellOpenId, int top_)
		{
			return await GetByCellOpenIdAsync(cellOpenId, top_, string.Empty, null);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByCellOpenId(int cellOpenId, int top_, TransactionManager tm_)
		{
			return GetByCellOpenId(cellOpenId, top_, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByCellOpenIdAsync(int cellOpenId, int top_, TransactionManager tm_)
		{
			return await GetByCellOpenIdAsync(cellOpenId, top_, string.Empty, tm_);
		}
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByCellOpenId(int cellOpenId, string sort_)
		{
			return GetByCellOpenId(cellOpenId, 0, sort_, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByCellOpenIdAsync(int cellOpenId, string sort_)
		{
			return await GetByCellOpenIdAsync(cellOpenId, 0, sort_, null);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByCellOpenId(int cellOpenId, string sort_, TransactionManager tm_)
		{
			return GetByCellOpenId(cellOpenId, 0, sort_, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByCellOpenIdAsync(int cellOpenId, string sort_, TransactionManager tm_)
		{
			return await GetByCellOpenIdAsync(cellOpenId, 0, sort_, tm_);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByCellOpenId(int cellOpenId, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`CellOpenId` = @CellOpenId", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@CellOpenId", cellOpenId, MySqlDbType.Int32));
			return Database.ExecSqlList(sql_, paras_, tm_, Gdt_bet_detailEO.MapDataReader);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByCellOpenIdAsync(int cellOpenId, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`CellOpenId` = @CellOpenId", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@CellOpenId", cellOpenId, MySqlDbType.Int32));
		...
		/// /// <param name = "cellType">格子内道具 1-钻石,2-地雷</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByCellType(int cellType)
		{
			return GetByCellType(cellType, 0, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByCellTypeAsync(int cellType)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByCellType(int cellType, TransactionManager tm_)
		{
			return GetByCellType(cellType, 0, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByCellTypeAsync(int cellType, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByCellType(int cellType, int top_)
		{
			return GetByCellType(cellType, top_, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByCellTypeAsync(int cellType, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByCellType(int cellType, int top_, TransactionManager tm_)
		{
			return GetByCellType(cellType, top_, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByCellTypeAsync(int cellType, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByCellType(int cellType, string sort_)
		{
			return GetByCellType(cellType, 0, sort_, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByCellTypeAsync(int cellType, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByCellType(int cellType, string sort_, TransactionManager tm_)
		{
			return GetByCellType(cellType, 0, sort_, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByCellTypeAsync(int cellType, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByCellType(int cellType, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Gdt_bet_detailEO>> GetByCellTypeAsync(int cellType, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`CellType` = @CellType", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@CellType", cellType, MySqlDbType.Int32));
		...
		/// /// <param name = "orderStatus">状态0-初始1-处理中2-成功3-失败4-已回滚5-异常且需处理6-异常已处理</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByOrderStatus(int orderStatus)
		{
			return GetByOrderStatus(orderStatus, 0, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByOrderStatusAsync(int orderStatus)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByOrderStatus(int orderStatus, TransactionManager tm_)
		{
			return GetByOrderStatus(orderStatus, 0, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByOrderStatusAsync(int orderStatus, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByOrderStatus(int orderStatus, int top_)
		{
			return GetByOrderStatus(orderStatus, top_, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByOrderStatusAsync(int orderStatus, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByOrderStatus(int orderStatus, int top_, TransactionManager tm_)
		{
			return GetByOrderStatus(orderStatus, top_, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByOrderStatusAsync(int orderStatus, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByOrderStatus(int orderStatus, string sort_)
		{
			return GetByOrderStatus(orderStatus, 0, sort_, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByOrderStatusAsync(int orderStatus, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByOrderStatus(int orderStatus, string sort_, TransactionManager tm_)
		{
			return GetByOrderStatus(orderStatus, 0, sort_, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByOrderStatusAsync(int orderStatus, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByOrderStatus(int orderStatus, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Gdt_bet_detailEO>> GetByOrderStatusAsync(int orderStatus, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`OrderStatus` = @OrderStatus", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@OrderStatus", orderStatus, MySqlDbType.Int32));
		...
		/// /// <param name = "recDate">记录时间</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByRecDate(DateTime? recDate)
		{
			return GetByRecDate(recDate, 0, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByRecDateAsync(DateTime? recDate)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByRecDate(DateTime? recDate, TransactionManager tm_)
		{
			return GetByRecDate(recDate, 0, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByRecDateAsync(DateTime? recDate, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByRecDate(DateTime? recDate, int top_)
		{
			return GetByRecDate(recDate, top_, string.Empty, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByRecDateAsync(DateTime? recDate, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByRecDate(DateTime? recDate, int top_, TransactionManager tm_)
		{
			return GetByRecDate(recDate, top_, string.Empty, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByRecDateAsync(DateTime? recDate, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByRecDate(DateTime? recDate, string sort_)
		{
			return GetByRecDate(recDate, 0, sort_, null);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByRecDateAsync(DateTime? recDate, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByRecDate(DateTime? recDate, string sort_, TransactionManager tm_)
		{
			return GetByRecDate(recDate, 0, sort_, tm_);
		}
		public async Task<List<Gdt_bet_detailEO>> GetByRecDateAsync(DateTime? recDate, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Gdt_bet_detailEO> GetByRecDate(DateTime? recDate, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Gdt_bet_detailEO>> GetByRecDateAsync(DateTime? recDate, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL(recDate.HasValue ? "`RecDate` = @RecDate" : "`RecDate` IS NULL", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			if (recDate.HasValue)
