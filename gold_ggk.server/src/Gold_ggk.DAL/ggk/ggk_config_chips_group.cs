...
namespace Gold_ggk.DAL
{
	...
	/// <summary>
	/// 实际下注筹码分组表
	/// 【表 ggk_config_chips_group 的实体类】
	/// </summary>
	[DataContract]
	[Obsolete]
	public class Ggk_config_chips_groupEO : IRowMapper<Ggk_config_chips_groupEO>
	{
		/// <summary>
		/// 构造函数 
		/// </summary>
		public Ggk_config_chips_groupEO()
		{
		}
		#region 主键原始值 & 主键集合
		...
		/// <return>实体对象</return>
		public Ggk_config_chips_groupEO MapRow(IDataReader reader)
		{
		...
		/// <param name = "reader">只进结果集流</param>
		/// <return>实体对象</return>
		public static Ggk_config_chips_groupEO MapDataReader(IDataReader reader)
		{
		    Ggk_config_chips_groupEO ret = new Ggk_config_chips_groupEO();
	...
	/// <summary>
	/// 实际下注筹码分组表
	/// 【表 ggk_config_chips_group 的操作类】
	/// </summary>
	[Obsolete]
	public class Ggk_config_chips_groupMO : MySqlTableMO<Ggk_config_chips_groupEO>
	{
		/// <summary>
		/// 表名
		/// </summary>
	    public override string TableName { get; set; } = "`ggk_config_chips_group`";
		...
		public Ggk_config_chips_groupMO(MySqlDatabase database) : base(database) { }
		/// <summary>
		/// 构造函数
		/// </summary>
		/// <param name = "connectionStringName">配置文件.config中定义的连接字符串名称</param>
		public Ggk_config_chips_groupMO(string connectionStringName = null) : base(connectionStringName) { }
	    /// <summary>
	    /// 构造函数
	    /// </summary>
	    /// <param name="connectionString">数据库连接字符串，如server=192.168.1.1;database=testdb;uid=root;pwd=root</param>
	    /// <param name="commandTimeout">CommandTimeout时间</param>
	    public Ggk_config_chips_groupMO(string connectionString, int commandTimeout) : base(connectionString, commandTimeout) { }
		...
		public override int Add(Ggk_config_chips_groupEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_); 
		}
		public override async Task<int> AddAsync(Ggk_config_chips_groupEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_); 
		}
	    private void RepairAddData(Ggk_config_chips_groupEO item, bool useIgnore_, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = useIgnore_ ? "INSERT IGNORE" : "INSERT";
			sql_ += $" INTO {TableName} (`ChipsID`, `BetTotalAmount`, `Group`) VALUE (@ChipsID, @BetTotalAmount, @Group);";
		...
		public int AddByBatch(IEnumerable<Ggk_config_chips_groupEO> items, int batchCount, TransactionManager tm_ = null)
		{
			var ret = 0;
	    ...
	    public async Task<int> AddByBatchAsync(IEnumerable<Ggk_config_chips_groupEO> items, int batchCount, TransactionManager tm_ = null)
	    {
	        var ret = 0;
	        foreach (var sql in BuildAddBatchSql(items, batchCount))
	        {
	    ...
	    private IEnumerable<string> BuildAddBatchSql(IEnumerable<Ggk_config_chips_groupEO> items, int batchCount)
		{
			var count = 0;
	        var insertSql = $"INSERT INTO {TableName} (`ChipsID`, `BetTotalAmount`, `Group`) VALUES ";
			var sql = new StringBuilder();
	        foreach (var item in items)
			{
				count++;
				sql.Append($"({item.ChipsID},{item.BetTotalAmount},'{item.Group}'),");
				if (count == batchCount)
				{
					count = 0;
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Remove(Ggk_config_chips_groupEO item, TransactionManager tm_ = null)
		{
			return RemoveByPK(item.ChipsID, item.BetTotalAmount, tm_);
		}
		public async Task<int> RemoveAsync(Ggk_config_chips_groupEO item, TransactionManager tm_ = null)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Put(Ggk_config_chips_groupEO item, TransactionManager tm_ = null)
		{
			RepairPutData(item, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutAsync(Ggk_config_chips_groupEO item, TransactionManager tm_ = null)
		{
			RepairPutData(item, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutData(Ggk_config_chips_groupEO item, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `ChipsID` = @ChipsID, `BetTotalAmount` = @BetTotalAmount, `Group` = @Group WHERE `ChipsID` = @ChipsID_Original AND `BetTotalAmount` = @BetTotalAmount_Original";
			paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@ChipsID", item.ChipsID, MySqlDbType.Int32),
				Database.CreateInParameter("@BetTotalAmount", item.BetTotalAmount, MySqlDbType.Int64),
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Put(IEnumerable<Ggk_config_chips_groupEO> items, TransactionManager tm_ = null)
		{
			int ret = 0;
			foreach (var item in items)
			{
		...
		public async Task<int> PutAsync(IEnumerable<Ggk_config_chips_groupEO> items, TransactionManager tm_ = null)
		{
			int ret = 0;
			foreach (var item in items)
			{
		...
		public bool Set(Ggk_config_chips_groupEO item, TransactionManager tm = null)
		{
			bool ret = true;
		...
		public async Task<bool> SetAsync(Ggk_config_chips_groupEO item, TransactionManager tm = null)
		{
			bool ret = true;
			if(GetByPK(item.ChipsID, item.BetTotalAmount) == null)
			{
		...
		/// <return></return>
		public Ggk_config_chips_groupEO GetByPK(int chipsID, long betTotalAmount, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(chipsID, betTotalAmount, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return Database.ExecSqlSingle(sql_, paras_, tm_, Ggk_config_chips_groupEO.MapDataReader);
		}
		public async Task<Ggk_config_chips_groupEO> GetByPKAsync(int chipsID, long betTotalAmount, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(chipsID, betTotalAmount, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return await Database.ExecSqlSingleAsync(sql_, paras_, tm_, Ggk_config_chips_groupEO.MapDataReader);
		}
		private void RepairGetByPKData(int chipsID, long betTotalAmount, out string sql_, out List<MySqlParameter> paras_, bool isForUpdate_ = false, TransactionManager tm_ = null)
		{
		...
		public List<Ggk_config_chips_groupEO> GetByChipsID(int chipsID)
		{
			return GetByChipsID(chipsID, 0, string.Empty, null);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByChipsIDAsync(int chipsID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByChipsID(int chipsID, TransactionManager tm_)
		{
			return GetByChipsID(chipsID, 0, string.Empty, tm_);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByChipsIDAsync(int chipsID, TransactionManager tm_)
		{
		...
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByChipsID(int chipsID, int top_)
		{
			return GetByChipsID(chipsID, top_, string.Empty, null);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByChipsIDAsync(int chipsID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByChipsID(int chipsID, int top_, TransactionManager tm_)
		{
			return GetByChipsID(chipsID, top_, string.Empty, tm_);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByChipsIDAsync(int chipsID, int top_, TransactionManager tm_)
		{
		...
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByChipsID(int chipsID, string sort_)
		{
			return GetByChipsID(chipsID, 0, sort_, null);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByChipsIDAsync(int chipsID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByChipsID(int chipsID, string sort_, TransactionManager tm_)
		{
			return GetByChipsID(chipsID, 0, sort_, tm_);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByChipsIDAsync(int chipsID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByChipsID(int chipsID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`ChipsID` = @ChipsID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@ChipsID", chipsID, MySqlDbType.Int32));
			return Database.ExecSqlList(sql_, paras_, tm_, Ggk_config_chips_groupEO.MapDataReader);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByChipsIDAsync(int chipsID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`ChipsID` = @ChipsID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@ChipsID", chipsID, MySqlDbType.Int32));
			return await Database.ExecSqlListAsync(sql_, paras_, tm_, Ggk_config_chips_groupEO.MapDataReader);
		}
		...
		/// /// <param name = "betTotalAmount">实际下注额</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByBetTotalAmount(long betTotalAmount)
		{
			return GetByBetTotalAmount(betTotalAmount, 0, string.Empty, null);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByBetTotalAmountAsync(long betTotalAmount)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByBetTotalAmount(long betTotalAmount, TransactionManager tm_)
		{
			return GetByBetTotalAmount(betTotalAmount, 0, string.Empty, tm_);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByBetTotalAmountAsync(long betTotalAmount, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByBetTotalAmount(long betTotalAmount, int top_)
		{
			return GetByBetTotalAmount(betTotalAmount, top_, string.Empty, null);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByBetTotalAmountAsync(long betTotalAmount, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByBetTotalAmount(long betTotalAmount, int top_, TransactionManager tm_)
		{
			return GetByBetTotalAmount(betTotalAmount, top_, string.Empty, tm_);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByBetTotalAmountAsync(long betTotalAmount, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByBetTotalAmount(long betTotalAmount, string sort_)
		{
			return GetByBetTotalAmount(betTotalAmount, 0, sort_, null);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByBetTotalAmountAsync(long betTotalAmount, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByBetTotalAmount(long betTotalAmount, string sort_, TransactionManager tm_)
		{
			return GetByBetTotalAmount(betTotalAmount, 0, sort_, tm_);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByBetTotalAmountAsync(long betTotalAmount, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByBetTotalAmount(long betTotalAmount, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Ggk_config_chips_groupEO>> GetByBetTotalAmountAsync(long betTotalAmount, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`BetTotalAmount` = @BetTotalAmount", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@BetTotalAmount", betTotalAmount, MySqlDbType.Int64));
		...
		/// /// <param name = "group"></param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByGroup(string group)
		{
			return GetByGroup(group, 0, string.Empty, null);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByGroupAsync(string group)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByGroup(string group, TransactionManager tm_)
		{
			return GetByGroup(group, 0, string.Empty, tm_);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByGroupAsync(string group, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByGroup(string group, int top_)
		{
			return GetByGroup(group, top_, string.Empty, null);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByGroupAsync(string group, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByGroup(string group, int top_, TransactionManager tm_)
		{
			return GetByGroup(group, top_, string.Empty, tm_);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByGroupAsync(string group, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByGroup(string group, string sort_)
		{
			return GetByGroup(group, 0, sort_, null);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByGroupAsync(string group, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByGroup(string group, string sort_, TransactionManager tm_)
		{
			return GetByGroup(group, 0, sort_, tm_);
		}
		public async Task<List<Ggk_config_chips_groupEO>> GetByGroupAsync(string group, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_config_chips_groupEO> GetByGroup(string group, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Ggk_config_chips_groupEO>> GetByGroupAsync(string group, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL(group != null ? "`Group` = @Group" : "`Group` IS NULL", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			if (group != null)
