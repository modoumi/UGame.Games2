...
namespace Gold_ggk.DAL
{
	...
	[DataContract]
	[Obsolete]
	public class Ggk_user_extEO : IRowMapper<Ggk_user_extEO>
	{
		/// <summary>
		/// 构造函数 
		/// </summary>
		public Ggk_user_extEO()
		{
			this.ChipsID = 0;
			this.RoundIdx = 0;
		...
		/// <return>实体对象</return>
		public Ggk_user_extEO MapRow(IDataReader reader)
		{
		...
		/// <param name = "reader">只进结果集流</param>
		/// <return>实体对象</return>
		public static Ggk_user_extEO MapDataReader(IDataReader reader)
		{
		    Ggk_user_extEO ret = new Ggk_user_extEO();
	...
	/// </summary>
	[Obsolete]
	public class Ggk_user_extMO : MySqlTableMO<Ggk_user_extEO>
	{
		/// <summary>
		/// 表名
		/// </summary>
	    public override string TableName { get; set; } = "`ggk_user_ext`";
		...
		public Ggk_user_extMO(MySqlDatabase database) : base(database) { }
		/// <summary>
		/// 构造函数
		/// </summary>
		/// <param name = "connectionStringName">配置文件.config中定义的连接字符串名称</param>
		public Ggk_user_extMO(string connectionStringName = null) : base(connectionStringName) { }
	    /// <summary>
	    /// 构造函数
	    /// </summary>
	    /// <param name="connectionString">数据库连接字符串，如server=192.168.1.1;database=testdb;uid=root;pwd=root</param>
	    /// <param name="commandTimeout">CommandTimeout时间</param>
	    public Ggk_user_extMO(string connectionString, int commandTimeout) : base(connectionString, commandTimeout) { }
		...
		public override int Add(Ggk_user_extEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_); 
		}
		public override async Task<int> AddAsync(Ggk_user_extEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_); 
		}
	    private void RepairAddData(Ggk_user_extEO item, bool useIgnore_, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = useIgnore_ ? "INSERT IGNORE" : "INSERT";
			sql_ += $" INTO {TableName} (`UserID`, `ChipsID`, `RoundIdx`, `FreeSpinCount`, `UserStatus`, `LastRecDate`) VALUE (@UserID, @ChipsID, @RoundIdx, @FreeSpinCount, @UserStatus, @LastRecDate);";
		...
		public int AddByBatch(IEnumerable<Ggk_user_extEO> items, int batchCount, TransactionManager tm_ = null)
		{
			var ret = 0;
	    ...
	    public async Task<int> AddByBatchAsync(IEnumerable<Ggk_user_extEO> items, int batchCount, TransactionManager tm_ = null)
	    {
	        var ret = 0;
	        foreach (var sql in BuildAddBatchSql(items, batchCount))
	        {
	    ...
	    private IEnumerable<string> BuildAddBatchSql(IEnumerable<Ggk_user_extEO> items, int batchCount)
		{
			var count = 0;
	        var insertSql = $"INSERT INTO {TableName} (`UserID`, `ChipsID`, `RoundIdx`, `FreeSpinCount`, `UserStatus`, `LastRecDate`) VALUES ";
			var sql = new StringBuilder();
	        foreach (var item in items)
			{
				count++;
				sql.Append($"('{item.UserID}',{item.ChipsID},{item.RoundIdx},{item.FreeSpinCount},{item.UserStatus},'{item.LastRecDate?.ToString("yyyy-MM-dd HH:mm:ss")}'),");
				if (count == batchCount)
				{
					count = 0;
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Remove(Ggk_user_extEO item, TransactionManager tm_ = null)
		{
			return RemoveByPK(item.UserID, item.ChipsID, tm_);
		}
		public async Task<int> RemoveAsync(Ggk_user_extEO item, TransactionManager tm_ = null)
		{
		...
		/// 按字段删除
		/// </summary>
		/// /// <param name = "freeSpinCount">JP累计值</param>
		/// <param name="tm_">事务管理对象</param>
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Put(Ggk_user_extEO item, TransactionManager tm_ = null)
		{
			RepairPutData(item, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutAsync(Ggk_user_extEO item, TransactionManager tm_ = null)
		{
			RepairPutData(item, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutData(Ggk_user_extEO item, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `UserID` = @UserID, `ChipsID` = @ChipsID, `RoundIdx` = @RoundIdx, `FreeSpinCount` = @FreeSpinCount, `UserStatus` = @UserStatus WHERE `UserID` = @UserID_Original AND `ChipsID` = @ChipsID_Original";
			paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@UserID", item.UserID, MySqlDbType.VarChar),
				Database.CreateInParameter("@ChipsID", item.ChipsID, MySqlDbType.Int32),
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Put(IEnumerable<Ggk_user_extEO> items, TransactionManager tm_ = null)
		{
			int ret = 0;
			foreach (var item in items)
			{
		...
		public async Task<int> PutAsync(IEnumerable<Ggk_user_extEO> items, TransactionManager tm_ = null)
		{
			int ret = 0;
			foreach (var item in items)
			{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutRoundIdxByPK(string userID, int chipsID, int roundIdx, TransactionManager tm_ = null)
		{
			RepairPutRoundIdxByPKData(userID, chipsID, roundIdx, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutRoundIdxByPKAsync(string userID, int chipsID, int roundIdx, TransactionManager tm_ = null)
		{
			RepairPutRoundIdxByPKData(userID, chipsID, roundIdx, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutRoundIdxByPKData(string userID, int chipsID, int roundIdx, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `RoundIdx` = @RoundIdx  WHERE `UserID` = @UserID AND `ChipsID` = @ChipsID";
			paras_ = new List<MySqlParameter>() 
			{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutUserStatusByPK(string userID, int chipsID, int userStatus, TransactionManager tm_ = null)
		{
			RepairPutUserStatusByPKData(userID, chipsID, userStatus, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutUserStatusByPKAsync(string userID, int chipsID, int userStatus, TransactionManager tm_ = null)
		{
			RepairPutUserStatusByPKData(userID, chipsID, userStatus, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutUserStatusByPKData(string userID, int chipsID, int userStatus, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `UserStatus` = @UserStatus  WHERE `UserID` = @UserID AND `ChipsID` = @ChipsID";
			paras_ = new List<MySqlParameter>() 
			{
		...
		public bool Set(Ggk_user_extEO item, TransactionManager tm = null)
		{
			bool ret = true;
		...
		public async Task<bool> SetAsync(Ggk_user_extEO item, TransactionManager tm = null)
		{
			bool ret = true;
			if(GetByPK(item.UserID, item.ChipsID) == null)
			{
		...
		/// <return></return>
		public Ggk_user_extEO GetByPK(string userID, int chipsID, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(userID, chipsID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return Database.ExecSqlSingle(sql_, paras_, tm_, Ggk_user_extEO.MapDataReader);
		}
		public async Task<Ggk_user_extEO> GetByPKAsync(string userID, int chipsID, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(userID, chipsID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return await Database.ExecSqlSingleAsync(sql_, paras_, tm_, Ggk_user_extEO.MapDataReader);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return></return>
		public int GetRoundIdxByPK(string userID, int chipsID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@UserID", userID, MySqlDbType.VarChar),
			...
			return (int)GetScalar("`RoundIdx`", "`UserID` = @UserID AND `ChipsID` = @ChipsID", paras_, tm_);
		}
		public async Task<int> GetRoundIdxByPKAsync(string userID, int chipsID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@UserID", userID, MySqlDbType.VarChar),
			...
			return (int)await GetScalarAsync("`RoundIdx`", "`UserID` = @UserID AND `ChipsID` = @ChipsID", paras_, tm_);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return></return>
		public int GetUserStatusByPK(string userID, int chipsID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@UserID", userID, MySqlDbType.VarChar),
			...
			return (int)GetScalar("`UserStatus`", "`UserID` = @UserID AND `ChipsID` = @ChipsID", paras_, tm_);
		}
		public async Task<int> GetUserStatusByPKAsync(string userID, int chipsID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@UserID", userID, MySqlDbType.VarChar),
			...
			return (int)await GetScalarAsync("`UserStatus`", "`UserID` = @UserID AND `ChipsID` = @ChipsID", paras_, tm_);
		}
		...
		public List<Ggk_user_extEO> GetByUserID(string userID)
		{
			return GetByUserID(userID, 0, string.Empty, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByUserIDAsync(string userID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByUserID(string userID, TransactionManager tm_)
		{
			return GetByUserID(userID, 0, string.Empty, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByUserIDAsync(string userID, TransactionManager tm_)
		{
		...
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByUserID(string userID, int top_)
		{
			return GetByUserID(userID, top_, string.Empty, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByUserIDAsync(string userID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByUserID(string userID, int top_, TransactionManager tm_)
		{
			return GetByUserID(userID, top_, string.Empty, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByUserIDAsync(string userID, int top_, TransactionManager tm_)
		{
		...
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByUserID(string userID, string sort_)
		{
			return GetByUserID(userID, 0, sort_, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByUserIDAsync(string userID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByUserID(string userID, string sort_, TransactionManager tm_)
		{
			return GetByUserID(userID, 0, sort_, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByUserIDAsync(string userID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByUserID(string userID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`UserID` = @UserID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@UserID", userID, MySqlDbType.VarChar));
			return Database.ExecSqlList(sql_, paras_, tm_, Ggk_user_extEO.MapDataReader);
		}
		public async Task<List<Ggk_user_extEO>> GetByUserIDAsync(string userID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`UserID` = @UserID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@UserID", userID, MySqlDbType.VarChar));
			return await Database.ExecSqlListAsync(sql_, paras_, tm_, Ggk_user_extEO.MapDataReader);
		}
		...
		/// /// <param name = "chipsID">下注id</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByChipsID(int chipsID)
		{
			return GetByChipsID(chipsID, 0, string.Empty, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByChipsIDAsync(int chipsID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByChipsID(int chipsID, TransactionManager tm_)
		{
			return GetByChipsID(chipsID, 0, string.Empty, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByChipsIDAsync(int chipsID, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByChipsID(int chipsID, int top_)
		{
			return GetByChipsID(chipsID, top_, string.Empty, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByChipsIDAsync(int chipsID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByChipsID(int chipsID, int top_, TransactionManager tm_)
		{
			return GetByChipsID(chipsID, top_, string.Empty, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByChipsIDAsync(int chipsID, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByChipsID(int chipsID, string sort_)
		{
			return GetByChipsID(chipsID, 0, sort_, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByChipsIDAsync(int chipsID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByChipsID(int chipsID, string sort_, TransactionManager tm_)
		{
			return GetByChipsID(chipsID, 0, sort_, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByChipsIDAsync(int chipsID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByChipsID(int chipsID, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Ggk_user_extEO>> GetByChipsIDAsync(int chipsID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`ChipsID` = @ChipsID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@ChipsID", chipsID, MySqlDbType.Int32));
		...
		/// /// <param name = "roundIdx">回合数，总翻拍次数</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByRoundIdx(int roundIdx)
		{
			return GetByRoundIdx(roundIdx, 0, string.Empty, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByRoundIdxAsync(int roundIdx)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByRoundIdx(int roundIdx, TransactionManager tm_)
		{
			return GetByRoundIdx(roundIdx, 0, string.Empty, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByRoundIdxAsync(int roundIdx, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByRoundIdx(int roundIdx, int top_)
		{
			return GetByRoundIdx(roundIdx, top_, string.Empty, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByRoundIdxAsync(int roundIdx, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByRoundIdx(int roundIdx, int top_, TransactionManager tm_)
		{
			return GetByRoundIdx(roundIdx, top_, string.Empty, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByRoundIdxAsync(int roundIdx, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByRoundIdx(int roundIdx, string sort_)
		{
			return GetByRoundIdx(roundIdx, 0, sort_, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByRoundIdxAsync(int roundIdx, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByRoundIdx(int roundIdx, string sort_, TransactionManager tm_)
		{
			return GetByRoundIdx(roundIdx, 0, sort_, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByRoundIdxAsync(int roundIdx, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByRoundIdx(int roundIdx, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Ggk_user_extEO>> GetByRoundIdxAsync(int roundIdx, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`RoundIdx` = @RoundIdx", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@RoundIdx", roundIdx, MySqlDbType.Int32));
		...
		/// /// <param name = "freeSpinCount">JP累计值</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByFreeSpinCount(int freeSpinCount)
		{
			return GetByFreeSpinCount(freeSpinCount, 0, string.Empty, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByFreeSpinCountAsync(int freeSpinCount)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByFreeSpinCount(int freeSpinCount, TransactionManager tm_)
		{
			return GetByFreeSpinCount(freeSpinCount, 0, string.Empty, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByFreeSpinCountAsync(int freeSpinCount, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByFreeSpinCount(int freeSpinCount, int top_)
		{
			return GetByFreeSpinCount(freeSpinCount, top_, string.Empty, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByFreeSpinCountAsync(int freeSpinCount, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByFreeSpinCount(int freeSpinCount, int top_, TransactionManager tm_)
		{
			return GetByFreeSpinCount(freeSpinCount, top_, string.Empty, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByFreeSpinCountAsync(int freeSpinCount, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByFreeSpinCount(int freeSpinCount, string sort_)
		{
			return GetByFreeSpinCount(freeSpinCount, 0, sort_, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByFreeSpinCountAsync(int freeSpinCount, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByFreeSpinCount(int freeSpinCount, string sort_, TransactionManager tm_)
		{
			return GetByFreeSpinCount(freeSpinCount, 0, sort_, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByFreeSpinCountAsync(int freeSpinCount, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByFreeSpinCount(int freeSpinCount, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Ggk_user_extEO>> GetByFreeSpinCountAsync(int freeSpinCount, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`FreeSpinCount` = @FreeSpinCount", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@FreeSpinCount", freeSpinCount, MySqlDbType.Int32));
		...
		/// /// <param name = "userStatus">1-正常 2-锁定3-异常</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByUserStatus(int userStatus)
		{
			return GetByUserStatus(userStatus, 0, string.Empty, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByUserStatusAsync(int userStatus)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByUserStatus(int userStatus, TransactionManager tm_)
		{
			return GetByUserStatus(userStatus, 0, string.Empty, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByUserStatusAsync(int userStatus, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByUserStatus(int userStatus, int top_)
		{
			return GetByUserStatus(userStatus, top_, string.Empty, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByUserStatusAsync(int userStatus, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByUserStatus(int userStatus, int top_, TransactionManager tm_)
		{
			return GetByUserStatus(userStatus, top_, string.Empty, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByUserStatusAsync(int userStatus, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByUserStatus(int userStatus, string sort_)
		{
			return GetByUserStatus(userStatus, 0, sort_, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByUserStatusAsync(int userStatus, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByUserStatus(int userStatus, string sort_, TransactionManager tm_)
		{
			return GetByUserStatus(userStatus, 0, sort_, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByUserStatusAsync(int userStatus, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByUserStatus(int userStatus, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Ggk_user_extEO>> GetByUserStatusAsync(int userStatus, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`UserStatus` = @UserStatus", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@UserStatus", userStatus, MySqlDbType.Int32));
		...
		/// /// <param name = "lastRecDate">记录最后j记录时间</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByLastRecDate(DateTime? lastRecDate)
		{
			return GetByLastRecDate(lastRecDate, 0, string.Empty, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByLastRecDateAsync(DateTime? lastRecDate)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByLastRecDate(DateTime? lastRecDate, TransactionManager tm_)
		{
			return GetByLastRecDate(lastRecDate, 0, string.Empty, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByLastRecDateAsync(DateTime? lastRecDate, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByLastRecDate(DateTime? lastRecDate, int top_)
		{
			return GetByLastRecDate(lastRecDate, top_, string.Empty, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByLastRecDateAsync(DateTime? lastRecDate, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByLastRecDate(DateTime? lastRecDate, int top_, TransactionManager tm_)
		{
			return GetByLastRecDate(lastRecDate, top_, string.Empty, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByLastRecDateAsync(DateTime? lastRecDate, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByLastRecDate(DateTime? lastRecDate, string sort_)
		{
			return GetByLastRecDate(lastRecDate, 0, sort_, null);
		}
		public async Task<List<Ggk_user_extEO>> GetByLastRecDateAsync(DateTime? lastRecDate, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByLastRecDate(DateTime? lastRecDate, string sort_, TransactionManager tm_)
		{
			return GetByLastRecDate(lastRecDate, 0, sort_, tm_);
		}
		public async Task<List<Ggk_user_extEO>> GetByLastRecDateAsync(DateTime? lastRecDate, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Ggk_user_extEO> GetByLastRecDate(DateTime? lastRecDate, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Ggk_user_extEO>> GetByLastRecDateAsync(DateTime? lastRecDate, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL(lastRecDate.HasValue ? "`LastRecDate` = @LastRecDate" : "`LastRecDate` IS NULL", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			if (lastRecDate.HasValue)
